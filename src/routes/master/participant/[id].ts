...
export const get =[ async (req: Request, res: Response) => {
  ...
  const data = await db1.tr_project_participant.findUnique({
    where: {
...
export const put = [authenticateJWT, async (req: Request, res: Response) => {
  try {
    const updateParticipant = await db1.tr_project_participant.update({
...
export const del = [authenticateJWT, async (req: Request, res: Response) => {
  try {
    ...
    await db1.tr_project_participant.update({
      where: { id: parseInt(req.params.id) },
