...
import moment from "moment";
  ...
  function formatCurrency(amount: number, currency: string = 'IDR', locale: string = 'id-ID'): string {
  ...
  function generateTableRows(data: any): string {
    return data
      .map(
        (technician: any) => `
          <tr>
            <td style="border: 1px solid #ccc; padding: 8px; text-align: left;">${technician.mst_authorization.employee_name}</td>
            <td style="border: 1px solid #ccc; padding: 8px; text-align: left;">${technician.mst_authorization.mst_manpower_cost.technician_level}</td>
            <td style="border: 1px solid #ccc; padding: 8px; text-align: left;">${formatCurrency(technician.mst_authorization.mst_manpower_cost.hourly)}</td>
          </tr>
        `
      )
      .join("");
